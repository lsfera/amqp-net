...
{
    internal class ConnectionStartOkPayload : MethodFramePayload
    {
        ...
        internal static ConnectionStartOkPayload Parse(IByteBuffer buffer)
        {
            return new ConnectionStartOkPayload(TableFieldValueCodec.Instance.Decode(buffer),
                                                ShortStringFieldValueCodec.Instance.Decode(buffer),
                                                LongStringFieldValueCodec.Instance.Decode(buffer),
                                                ShortStringFieldValueCodec.Instance.Decode(buffer));
        }

        internal ConnectionStartOkPayload(Table clientProperties,
                                          String mechanism,
                                          String response,
                                          String locale)
        {
